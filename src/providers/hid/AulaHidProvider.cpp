...
namespace
{
    ...
    // 新协议（0x10/56），若响应全 0 / 无效再尝试紧凑长度（0x10/48），最后回退
    // 旧协议（0xA0），以新设备优先。
    ...
    constexpr DeviceInfoProtocol kProtoNewCompact = {0x10, 48, true}; // 新协议紧凑设备
    ...
    // workMode / battery / chargeStatus 在 payload 中的偏移（两代协议一致）。
    constexpr size_t kWorkModeOffset = 16;
    ...
    constexpr AulaDeviceEntry kAulaDevices[] = {
        ...
        {0x810D, 0x0000, 0x0000},
        {0x8801, 0x0000, 0x0000},
        {0x8802, 0x0000, 0x0000}, // AJAZZ MK87PRO
    ...
    // 让上层回退到另一种协议。判定标准：自报 vid 或 pid 非 0，或 workMode /
    // battery / chargeStatus 非 0，
    // 任一成立即视为有效。
    bool deviceInfoLooksValid(const std::vector<unsigned char> &payload)
    {
        ...
        const int workMode = payload[kWorkModeOffset];
        const int battery = payload[kBatteryOffset];
        const int chargeStatus = payload[kChargeStatusOffset];
        return reportedVid != 0 || reportedPid != 0 || workMode != 0 ||
               battery != 0 || chargeStatus != 0;
    }

    bool isWiredWorkMode(int workMode)
    {
        return workMode == 0;
    }
                                  ...
                                  const std::wstring &id)
    {
        ...
        LOG_VERBOSE_W(L"[HID]     proto=0x" + std::wstring{toHex4(proto.cmd)} +
                      ...
                      L" workMode=" + std::to_wstring(workMode) +
        ...
        device.percentage = wired ? -1 : battery;
        device.level = wired ? BatteryLevel::Unknown : levelFromPercentage(battery);
...
std::vector<BatteryDevice> AulaHidProvider::readDevices()
{
                            ...
                            const AulaDeviceEntry *entry) -> std::optional<BatteryDevice>
    {
        ...
        try
        {
            ...
            // 若响应全 0 / 无效或无响应，尝试紧凑长度，再回退旧协议（cmd=0xA0）。
            //
            ...
            else
            {
                if (payload)
                {
                    LOG_VERBOSE_W(L"[HID]   new proto(0x10) response looks invalid "
                                  L"(all-zero), falling back to compact proto(0x10)");
                }
                payload = getDeviceInfo(dev, kProtoNewCompact, outDataLen, inDataLen);
                if (payload && deviceInfoLooksValid(*payload))
                {
                    usedProto = &kProtoNewCompact;
                }
                else
                {
                    if (payload)
                    {
                        LOG_VERBOSE_W(L"[HID]   compact proto(0x10) response looks invalid "
            ...
            else
            {
                ...
                LOG_VERBOSE_W(L"[HID]   " + name +
                              ...
                              L"(tried proto 0x10/56, 0x10/48, then 0xA0)");
            }
        }
