...
//       * 通配簇（78 个 PID，含 0xFEFE/F87 Pro Dongle）——不限定 usage，取该 PID 下
...
//   - 字段：payload[17] = battery(0-100)，payload[18] = chargeStatus(≠0 即充电)；
//     payload[4..7] = 设备自报 vid/pid（小端，仅用于日志）。
//
// —— 双协议支持（重要）——
// AULA / 关联品牌存在两代固件协议，命令号、contentSize、尾包标记都不同，
// readDevices() 对每个候选接口先尝试新协议，无效再回退旧协议：
//   - 新协议（F87ProV2D 等新设备）：
//       cmd = 0x10, contentSize = 56, 请求头 byte[6] = 尾包标志（最后一包置 1）。
//   - 旧协议（早期型号）：
